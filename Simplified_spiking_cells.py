...
class Spiking_cells:
    ...
    def PLASTICITY(self, ind_dend, SPIKE):
        ...
        if SPIKE:       
            ...
            #print('U change:', p_EXT_U-self.p_EXT_U[ind_dend])
            #print('R change:', p_EXT_R-self.p_EXT_R[ind_dend])
        else:
            self.p_EXT_U[ind_dend]-=(p_EXT_U-r_EXT)/10. #Potentiation Recovery time constants
            self.p_INH_U[ind_dend]-=(p_INH_U-r_INH)/20. 
            self.p_EXT_R[ind_dend]-=(p_EXT_R-1)/100.     #Depression Recovery time constants
            self.p_INH_R[ind_dend]-=(p_INH_R-1)/50.
